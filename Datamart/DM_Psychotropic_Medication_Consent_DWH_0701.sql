...
CREATE OR REPLACE TABLE DATAMART.DM_PSYCHOTROPIC_MEDICATION_CONSENT AS
...
MEDICATION_CLASSIFICATION_CODE_AV_ID,
MEDICATION_CLASSIFICATION_CODE_DESC
        FROM DWH.FACT_MEDICATION_HEALTH_BEHAVIOR FMHB
         INNER JOIN DWH.DIM_MEDICATION_HEALTH_BEHAVIOR_INFO FMHBI
         ON FMHB.MEDICATION_HEALTH_BEHAVIOR_INFO_ID = FMHBI.MEDICATION_HEALTH_BEHAVIOR_INFO_ID
         INNER JOIN DWH.FACT_MEDICATIONS FM ON FMHB.MED_MED_ID = FM.MED_ID
         INNER JOIN DWH.DIM_MEDICATIONS_INFO DMI ON FM.MEDICATIONS_INFO_ID = DMI.MEDICATIONS_INFO_ID
         LEFT JOIN DWH.DIM_T_PERSONS per ON (FMHB.PERSON_PERSON_PATIENT_ID = PER.PERSON_ID)
         WHERE 
        ...
        AND EXISTS
        (SELECT 'x'
            FROM DWH.FACT_PERSON_ORG_INVOLVEMENT POI
                INNER JOIN DWH.FACT_PERSON_ROLE_ASSIGNMENTS PRA 
                ON POI.POI_ID = PRA.POI_POI_ID
                INNER JOIN DWH.DIM_PERSON_ROLE_ASSIGNMENTS_INFO DPR 
...
CUSTODY_DATA AS (
      ...
      FROM (SELECT CUS_ID,
            ...
            FROM DWH.FACT_CUSTODIES FC 
            INNER JOIN DWH.DIM_CUSTODIES_INFO DCI 
...
CASE_INFO AS (
      ...
      FROM DWH.FACT_CASES C 
      INNER JOIN DWH.DIM_CASES_INFO DCI 
...
CASE_ASSIGNMENT AS (
    ...
    FROM DWH.fact_staff_person_office_assigns spoa
    JOIN DWH.dim_staff_person_office_assigns_info dpoa
